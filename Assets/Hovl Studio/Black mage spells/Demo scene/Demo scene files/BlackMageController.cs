...
public class BlackMageController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown("4"))
        {
            if (fastSkillrefresh[3] == false)
                StartCoroutine(FastPlay(3, 0, 3.5f));
        }
    ...
    public void SecondLayer(bool weight)
    {
        if (weight == true)
    ...
    public IEnumerator FastPlay(int EffectNumber, float castDelay, float endDelay)
    {
        ...
        if (EffectNumber == 3) StartCoroutine(cameraShaker.Shake(0.3f, 5, 0.5f, 0));
        if (EffectNumber == 2)
        {
            casting = true;
            canMove = false;
            ...
            StartCoroutine(cameraShaker.Shake(0.3f, 5, 1f, 0.4f));
        }
        ...
        if (PrefabsCast[EffectNumber] && TargetMarker)
        {
            ...
            while (true)
            {
                ...
                if (Input.GetMouseButtonDown(0) && casting == false)
                {
                    ...
                    if (rotateState == false)
                    {
                        StartCoroutine(RotateToTarget(fireRate, hit.point));
                    }   
                    secondLayerWeight = Mathf.Lerp(secondLayerWeight, 1, Time.deltaTime * 10);
                    casting = true;
    ...
    IEnumerator OnCast(int EffectNumber)
    {
        while (true)
        {
            if (casting)
            {
                ...
                if (EffectNumber == 1) StartCoroutine(cameraShaker.Shake(0.6f, 6, 0.3f, 0.6f));
                if (EffectNumber == 3) StartCoroutine(cameraShaker.Shake(0.6f, 6, 0.3f, 1.45f));
                if (EffectNumber == 4 || EffectNumber == 7) StartCoroutine(cameraShaker.Shake(0.6f, 6, 0.4f, 0.2f));
    ...
    {
        if (EffectNumber != 1 && EffectNumber != 4 && EffectNumber != 7)
        {
        ...
        while (true)
        {
            if (casting)
            {
                ...
                //Activate animation
                if (EffectNumber == 4 || EffectNumber == 7)
                {
                    SecondLayer(true);
                }
                if (EffectNumber == 0 || EffectNumber == 5)
                {
